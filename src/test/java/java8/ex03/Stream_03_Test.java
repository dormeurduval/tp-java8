...
public class Stream_03_Test {
    ...
    public void test_joining() throws Exception {
        ...
        String result = customers.stream().map(p->p.getFirstname()).sorted()
    ...
    public void test_partitionning() throws Exception {
        ...
        Map<Boolean, List<Pizza>> result=pizzas.stream().collect(Collectors.partitioningBy(p->
        p.getName().startsWith("L")));
